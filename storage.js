'use strict';
/* ============================================================
 * CANIVETE — armazenamento de arquivos
...
/* Gera nome único e grava (disco ou S3). Retorna o nome do arquivo. */
...
async function servir(req, res, nome) {
  ...
  if (!fs.existsSync(p)) return res.status(404).json({ erro: 'Não encontrado' });
  res.setHeader('Content-Type', mimeDe(nome));
  fs.createReadStream(p).pipe(res);
}

module.exports = { s3Ativo, salvar, remover, servir, UPLOAD_DIR, TMP_DIR, mimeDe };

#!/usr/bin/env python3
"""Cria os registros DNS do Canivete no Cloudflare (zona dtxnet.top).
Registros: painel (membros) e panadm (admin) → 187.127.48.130
Requisito: token Cloudflare com Zone > DNS > Edit (/home/hermeswebui/.cf_api_token)
"""
...
IP = "187.127.48.130"
ZONA_NOME = "dtxnet.top"
SUBDOMINIOS = [
    (os.environ.get("DOMINIO_MEMBROS", "painel.dtxnet.top"), "Canivete — membros"),
    (os.environ.get("PAINEL_ADM_HOSTS", "panadm.dtxnet.top"), "Canivete — painel admin"),
]

# 1. descobre a zona dtxnet.top
s, d = req(f"https://api.cloudflare.com/client/v4/zones?name={ZONA_NOME}", CF)
if not d.get("success") or not d.get("result"):
    print("ERRO: zona", ZONA_NOME, "não encontrada:", json.dumps(d)[:300])
    raise SystemExit(1)
